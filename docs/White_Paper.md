...
The **PE-MsBV** (Priority-Encoded Multi-stage Bit Vector) implements the interception pipeline per AEP: *“符合优先级编码的多级位向量（PE-MsBV）流水线”*.
...
1. **Folded Checksum Verification** — Recompute expected checksum; compare.
2. **Epoch Governance Clock Check** — Bounded time-window comparison.
3. **MsBV+ Policy Lookup** — Single table access (`policy_id in msbv_table`).
...
Per AEP Detectability Check: *“方案必须包含渐进收敛响应指纹（I/O Fingerprint）”*.
...
The 4-bit RLCP field reserves space for the **RLCP Logical Skeleton Sub-manifold** (RLCP 逻辑骨架子流形), utilizing the **Fisher Information Matrix (FIM)** for adaptive mask generation. Per AEP: *“代谢: 基于费舍尔信息矩阵（FIM）识别的拓扑保持代谢协议（RLCP）”*.

### 5.2 Hardware Neutrality

Per AEP: *“严禁绑定特定 CPU 指令，统一使用上位物理描述”*.
