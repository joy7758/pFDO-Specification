...
class SearchResponse(BaseModel):
    ...
    size: conint(ge=0) = Field(..., description='size: the number of results across all pages')
    results: List = Field(..., description='a list of results, each of which is either a string (the object id) or the default serialization of an object omitting element data.')
