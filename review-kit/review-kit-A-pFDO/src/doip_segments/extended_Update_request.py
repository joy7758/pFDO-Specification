...
class Attributes(BaseModel):
    record: Dict[str, Any] = Field(..., description='record: the JSON object containing all type-value-pairs that are used to override the entries in the PID record.')
...
class ExtendedUpdateRequest(BaseModel):
    ...
    operationId: Literal['0.DOIP/Op.Extended-Update'] = Field(..., description='operationId: the identifier of the operation to be performed.')
    attributes: Optional[Attributes] = Field(None, description='attributes: mandatory JSON object to specify the type-value pairs for the PID record')
